...
function readme_parser( $paras = '', $content = '' ) {
    ...
    extract( shortcode_atts( array( 'exclude' => '', 'ext' => '', 'hide' => '', 'scr_url' => '', 'scr_ext' => '' , 'target' => '_blank', 'nofollow' => '', 'ignore' => '', 'cache' => '', 'version' => '', 'mirror' => '', 'links' => 'bottom', 'name' => '' ), $paras ) );

    // Get cached output

    $result = false;
    if ( is_numeric( $cache ) ) {
        $cache_key = 'arp_' . md5( $exclude . $ext . $hide . $scr_url . $scr_ext . $target . $nofollow . $ignore . $cache . $version . $mirror . $content );
    ...
    if ( !$result ) {
        ...
        if ( $file_data !== false ) {
            ...
            for ( $i = 0; $i < $count; $i++ ) {
                ...
                if ( ( strtolower( $section ) == $links ) && ( $section != $prev_section ) ) {
                    if ( !arp_is_it_excluded( 'links', $exclude ) ) { $file_array[ $i ] = arp_display_links( $download, $target, $nofollow, $version, $mirror, $plugin_name ) . $file_array[ $i ]; }
                }
                ...
                if ( $add_to_output ) {
                    ...
                    if ( ( substr( $file_array[ $i ], 0, 13 ) == 'Contributors:' ) or ( substr( $file_array[ $i ], 0, 12 ) == 'Donate link:' ) or ( substr( $file_array[ $i ], 0, 5 ) == 'Tags:' ) or ( substr( $file_array[ $i ], 0, 18 ) == 'Requires at least:' ) or ( substr( $file_array[ $i ], 0, 13 ) == 'Tested up to:' ) or ( substr( $file_array[ $i ], 0, 11 ) == 'Stable tag:' ) ) {
                        ...
                        if ( ( substr( $file_array[ $i ], 0, 18 ) == 'Requires at least:' ) && ( arp_is_it_excluded( 'requires', $exclude ) ) ) { $add_to_output = false; }

                        if ( ( substr( $file_array[ $i ], 0, 13 ) == 'Tested up to:' ) && ( arp_is_it_excluded( 'tested', $exclude ) ) ) { $add_to_output = false; }
                ...
                if ( ( $section == 'Screenshots' ) && ( $add_to_output ) && ( $screenshot_url != '' ) ) {
                    if ( substr( $file_array[ $i ], 0, strlen( $screenshot ) + 2 ) == $screenshot . '. ' ) {
                        ...
                        if ( !$ext ) {
                            ...
                            $file_array[ $i ] = arp_report_error( sprintf( __( 'Could not find %s image file', 'wp-readme-parser' ), 'screenshot-' . $screenshot ), 'Artiss README Parser', false );
            ...
            if ( ( !arp_is_it_excluded( 'links', $exclude ) ) && ( $links == 'bottom' ) ) { $file_combined .= arp_display_links( $download, $target, $nofollow, $version, $mirror, $plugin_name ); }
            ...
            for ( $i = 0; $i < $count; $i++ ) {
                ...
                // If Artiss Content Reveal plugin is active
                ...
                if (function_exists('simple_content_reveal')) {
                    ...
                    if ( substr( $file_array[ $i ], 0, 4 ) == '<h2>' ) {
                        ...
                        // Call Artiss Content Reveal with heading details and replace current line
            ...
            if ( ( strlen( $file_data[ 'file' ] ) > 0 ) && ( substr_count( $file_data[ 'file' ], "\n" ) == 0 ) ) {
                $my_html = arp_report_error( __( 'Malformed README file - no carriage returns found', 'wp-readme-parser' ), 'Artiss README Parser', false );
            } else {
                $my_html = arp_report_error( __( 'README file could not be found or is malformed', 'wp-readme-parser' ) . ' - ' . $plugin_url, 'Artiss README Parser', false );
            }
        }

        // Send the resultant code back, plus encapsulating DIV and version comments

        $content = '<!-- Artiss README Parser v' . artiss_readme_parser_version . " | http://www.artiss.co.uk/readme-parser -->\n<div class=\"np-notepad\">" . $my_html . "</div>\n<!-- End of Artiss README Parser code -->\n";
...
function readme_banner( $paras = '', $content = '' ) {
    ...
    if ( $content == '' ) {
        ...
        $output = arp_report_error( __( 'No plugin name was supplied for banner', 'wp-readme-parser' ), 'Artiss README Parser', false );

    } else {

        // Build the banner URL
        ...
        // Check the banner exists - if it does, return it

        $img_check = arp_check_img_exists( $url, 'png' );
        ...
        if ( !$img_check ) {
            ...
            $output = '<div style="max-width: 100%;"><img src="' . $url . 'png" alt="' . $content . ' Banner" title="' . $content . ' Banner" /></div>';
    ...
    if ( !$result ) {
        ...
        if ( $file_data !== false ) {
            ...
            for ( $i = 0; $i < $count; $i++ ) {
                ...
                if ( substr( $file_array[ $i ], 0, 11 ) == 'Stable tag:' ) { $version = substr( $file_array[ $i ], 12 ); }
            }
            ...
            $output = arp_report_error( __( 'README file could not be found or is malformed', 'wp-readme-parser' ) . ' - ' . $name, 'Artiss README Parser', false );
        }
    } else {
    ...
    if ( $output == '' ) {
        ...
        if ( $data == 'download' ) {
            if ( ( $plugin_name != '' ) && ( $version != '' ) ) {
                ...
                $output = arp_report_error( __( 'The name and/or version number could not be found in the README', 'wp-readme-parser' ), 'Artiss README Parser', false );
            }
        }
        ...
        if ( $data == 'version' ) {
            if ( $version != '' ) {
                ...
                $output = arp_report_error( __( 'Version number not found in the README', 'wp-readme-parser' ), 'Artiss README Parser', false );
            }
        }
        ...
        if ( $data == 'forum' ) {
            if ( $plugin_name != '' ) {
                ...
                $output = arp_report_error( __( 'Plugin name not supplied', 'wp-readme-parser' ), 'Artiss README Parser', false );
            }
        }
        ...
        if ( $output == '' ) { $output = arp_report_error( __( 'The data parameter was invalid or missing', 'wp-readme-parser' ), 'Artiss README Parser', false ); }
